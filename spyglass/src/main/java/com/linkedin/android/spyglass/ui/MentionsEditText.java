...
public class MentionsEditText extends EditText implements TokenSource {
    ...
    public String getTextWithBusinessLogicString(){

        Editable text = getMentionsText();
        ...
        if(spans.length == 0) {
            ...
            for (MentionSpan span: spans) {
                ...
                String logicStr = span.getBusinessLogicString();
                int start = sb.getSpanStart(span);
                int end = sb.getSpanEnd(span);

//                Log.i("MentionsEditText","cursor:"+cursor);
//                Log.i("MentionsEditText","sb.length():"+sb.length());
//                Log.i("MentionsEditText","DisplayStr:"+span.getDisplayString());
//                Log.i("MentionsEditText","logicStr:"+logicStr);
//                Log.i("MentionsEditText","start:"+start);
//                Log.i("MentionsEditText","end:"+end);

                CharSequence subSequence = sb.subSequence(cursor, start);
//                Log.i("MentionsEditText","subSequence:"+subSequence);

                result.append(subSequence);
                result.append(logicStr);

//                Log.i("MentionsEditText","result:"+result.toString());
                cursor = end;
            }
            ...
            if(cursor < sb.length()-1) {
                ...
                String lastSubSequence = sb.subSequence(cursor, sb.length()).toString();
//                Log.i("MentionsEditText","lastSubSequence:"+lastSubSequence);


                result.append(lastSubSequence);
            }
    ...
    public void insertMentionInternal(@NonNull Mentionable mention, int start, int end) {
        // Insert the span into the editor
